class Source < ActiveRecord::Base
  ...
  def self.cached_sources_crafting_allowed
    data = Rails.cache.fetch("Source.cached_sources_crafting_allowed")
    if data == nil then
    data = Source.find(:all, :conditions => ["crafting_allowed = ?", true], :select => "id, description, crafting_allowed", :order => "description")
    Rails.cache.write("Source.cached_sources_crafting_allowed", data)
    end
    return data
  end
  ...
  def self.clear_cached_source(source_id)
    ...
    Rails.cache.clear("Source.cached_sources_crafting_allowed")
  end
